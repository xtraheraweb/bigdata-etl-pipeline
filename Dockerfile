...
CMD ["bash", "-c", "pip install apache-airflow-providers-docker && AIRFLOW__DATABASE__SQL_ALCHEMY_CONN=sqlite:////app/airflow.db airflow db init && AIRFLOW__DATABASE__SQL_ALCHEMY_CONN=sqlite:////app/airflow.db airflow users create --username admin --firstname Admin --lastname User --role Admin --email admin@example.com --password admin && AIRFLOW__DATABASE__SQL_ALCHEMY_CONN=sqlite:////app/airflow.db airflow standalone"]
